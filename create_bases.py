...
def create(train_max_size, imgs_per_product=5, test_entropy=0.3, test_size=1.0):
    ...
    extra_test_dir = os.path.join(script_dir, 'test_extra')

    if os.path.isdir(train_dir):
        shutil.rmtree(train_dir)
    if os.path.isdir(test_dir):
        shutil.rmtree(test_dir)
    if os.path.isdir(extra_test_dir):
        shutil.rmtree(extra_test_dir)


    os.mkdir(train_dir)
    os.mkdir(test_dir)
    os.mkdir(extra_test_dir)
    ...
    unkown_imgs_amnt = train_max_size * test_size
    if (test_entropy < 1.0):
    ...
    while(count_unknown < unkown_imgs_amnt and i < len(unknown_products_dirs)):
        ...
        i+=1
        imgs = os.listdir(os.path.join(base_folder, unk_product))[:4]
        extra_imgs = os.listdir(os.path.join(base_folder, unk_product))[4:]
        count_unknown += len(imgs)
        os.mkdir(os.path.join(test_dir, unk_product))
        if (len(extra_imgs) > 0):
            os.mkdir(os.path.join(extra_test_dir, unk_product))
        for img in imgs:
            shutil.copy(os.path.join(base_folder, unk_product, img), os.path.join(test_dir, unk_product, img))
        for img in extra_imgs:
            shutil.copy(os.path.join(base_folder, unk_product, img), os.path.join(extra_test_dir, unk_product, img))
